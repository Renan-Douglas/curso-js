let num = document.querySelector('#fnum')
let lista = document.querySelector('#flista')
let res = document.querySelector('#res')
let valores = []

function isNumero(n) {

    if (Number(n) >= 1 && Number(n) <= 100) {

        return true

    } else {

        return false

    }

}

function inLista(n, l) {

    if (l.indexOf(Number(n)) != -1) {

        return true

    } else {

        return false

    }

}

function adicionar() {

    if (isNumero(num.value) && !inLista(num.value, valores)) {

        valores.push(Number(num.value))

        let item = document.createElement('option')

        item.text = `Valor ${num.value} adicionado.`

        lista.appendChild(item)

        res.innerHTML = ''

    } else  {

        if (num.value > 100) {

            alert('Número acima do valor máximo')

        } else if (num.value < 1) {

            alert('Número abaixo do valor mínimo')

        } else {

            alert('Valor invalido ou já adicionado a lista')

        }

    }

    num.value = ''

    num.focus()

}

function finalizar() {

    if (valores.length == 0) {

        alert('Erro, vetor vazio. Adicione valores para prosseguir')

    } else {

        let tot = valores.length
        let maior = valores[0]
        let menor = valores[0]
        let soma = 0
        let media = 0

        for (let pos in valores) {

            soma += valores[pos]

            if (valores[pos] > maior) {

                maior = valores[pos]

            } if (valores[pos] < menor) {

                menor = valores[pos]

            }

        }

        media = soma / tot

        res.innerHTML = ''
        res.innerHTML += `<p>Ao todo, temos ${tot} número cadastrados.</p>`
        res.innerHTML += `<p>O maior valor informado é o ${maior}.</p>`
        res.innerHTML += `<p>O menor valor informado é o ${menor}.</p>`
        res.innerHTML += `Somando todos os valores o resultado é ${soma}`
        res.innerHTML += `<p>A média dos valores digitados é ${media}</p>`


    }

}